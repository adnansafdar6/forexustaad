                ...
                <div class="card">
                    ...
                    <div class="col-12 col-sm-6 col-md-12 mt-6  pt-2">
                        <form class="form form-vertical" action="{{ $action }}" method="POST" enctype="multipart/form-data">
                            <input type="hidden" value="{{ $favicon->img }}" name="image">
                            @csrf
                            <input type="hidden" value="PUT" name="_method">
                            <div class="row">
                            <div class="col-6 ">
                                <label for="img" style="margin-bottom: 10px;">FavIcon</label>
                                <input type="file" id="img" class="form-control" name="img"
                                       value="{{ old('img', $favicon->img) }}" {{ (($favicon->id == 0 ) ? 'required' : '') }}>
                                @if($errors->has('img'))
                                ...
                                <div class="col-6">
                                    @if(!is_null($favicon->img))
                                        <img class="ml-4 mb-3" src="{{ asset($favicon->img) }}" style="width: 100px; height: 100px;"
                                             alt="no image">
                                    @endif
                                </div>
                                <div class="row ml-2 mt-4">
                                    <div class="col-2 ">
                                        <input class="form-check-input" id="is_active" type="checkbox"
                                               name="is_active" {{ old('is_active', $favicon->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="col-8 d-flex justify-content-end mt-4">
                                <button type="submit" class="btn btn-primary me-1 mb-1">
                                    {{ $favicon->id != 0 ? 'Save Changes' : 'Add FavIcon' }}
                                </button>
                            </div>
                            </div>
                        </form>
                                    ...
                                    <tr>
                                        ...
                                        <td class="width-15">
                                            @if($value->is_active == 1)
                                                <a class="pr-2"
                                                   href="{{ route('admin.favicon.changeStatus', $value->id) }}"
                                            ...
                                            @endif
                                            <a class="pr-2" href="{{ route('admin.favicon.edit', $value->id) }}"
                                               title="Edit"><i class="fa-solid fa-pen-to-square" style="color: #68ee6a;"></i></a>
                                            <a href="javascript:{};"
                                               data-url="{{ route('admin.favicon.destroy',  $value->id) }}"
